...
anova_fatorial_trat <- anova_fatorial %>% 
  dplyr::mutate(Presença = dplyr::case_match(TRAT1,
                ...
                Área = dplyr::case_match(TRAT2,
                                         "C" ~ "Floresta Nativa",
                                             .default = "Plantação") %>% 
                  forcats::fct_relevel(c("Plantação", "Floresta Nativa")),
                Riqueza = RESP) %>% 
  dplyr::select(4:6)
...
anova_med_rep_trat <- anova_med_rep %>% 
  tidyr::pivot_longer(cols = dplyr::contains("Var"),
                      names_to = "Área",
                      values_to = "Riqueza") %>% 
  dplyr::mutate(Área = dplyr::case_match(Área,
                                         "Var1" ~ "Floresta Nativa",
                                         "Var2" ~ "Restauração",
                                         "Var3" ~ "Plantação") %>% 
                  forcats::fct_relevel(c("Plantação", "Restauração", "Floresta Nativa")))
...
```{r}
modelo_anova_fat <- aov(Riqueza ~ Presença * Área, data = anova_fatorial_trat)
```
...
anova_fat_flex %>% 
  flextable::save_as_docx(path = "tabela_anova_fatorial.docx")
```
...
anova_fatorial_trat %>% 
  ggplot(aes(Presença, Riqueza, fill = Área)) +
...
ggsave("anova_fatorial.png", height = 10, width = 12)
```
...
```{r}
modelo_anova_med_rep <- aov(Riqueza ~ Área + Error(Medidas), data = anova_med_rep_trat)
```
...
anova_med_rep_flextable %>% 
  flextable::save_as_docx(path = "tabela_medidas_repetidas.docx")
```
...
anova_med_rep_trat %>% 
  ggplot(aes(Medidas %>% factor(), Riqueza, fill = Área)) +
  ...
  labs(x = "Grupo de pesquisadores") +
  guides(fill = guide_legend(title.position = "top",
                             title.hjust = 0.5))


ggsave("anova_med_rep.png", height = 10, width = 12)
```
